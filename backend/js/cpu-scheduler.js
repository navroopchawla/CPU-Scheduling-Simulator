...
$(document).ready(function () {
	...
	function progressBar() {
		...
		this.displayBar = function () {
			...
			var tatSum = 0;
			var maxWait = 0;
			for (var k = 0; k < processArray.length; k++) {
				var tat = processArray[k].finishTime - processArray[k].arrivalTime;
				tatSum += tat;
				var w = waitTimes[k];
				if (w > maxWait) maxWait = w;
			}
			var avgTat = processArray.length
				? Math.round((tatSum / processArray.length) * 10000) / 10000
				: 0;

			window.lastSimulationSnapshot = {
				...
				throughput: this.sum > 0
					? Math.round((processArray.length / this.sum) * 1000) / 1000
					: 0,
				segments: this.names.slice(),
				segmentLengths: this.indexes.slice()
			};

			if (window.SchedulerDynamic && typeof window.SchedulerDynamic.onSimulationUpdate === "function") {
				window.SchedulerDynamic.onSimulationUpdate(window.lastSimulationSnapshot);
			}

			if (window.SchedulerDynamic && window.SchedulerDynamic.animateGantt !== false) {
				staggerGanttAnimation();
			}
		}
	}

	function staggerGanttAnimation() {
		var bars = $(".progress .progress-bar");
		bars.each(function (i, el) {
			var target = $(el).attr("data-target-width");
			setTimeout(function () {
				$(el).css("width", target + "%");
			}, 60 + i * 90);
		});
	...
	function addToBar(processName, percent, start, duration, index) {
		...
		$("#bar_" + index).attr("data-target-width", percent);
		var useAnimation = window.SchedulerDynamic && window.SchedulerDynamic.animateGantt !== false;
		if (useAnimation) {
			$("#bar_" + index).css("width", "0%").addClass("gantt-segment-animate");
		} else {
	...
	*/
	...
	var CPU_PRESETS = {
		fcfsClassic: {
			...
			quantum: 2,
			processes: [
				{ arrive: 0, burst: 8, priority: 2 },
				{ arrive: 1, burst: 4, priority: 1 },
				{ arrive: 2, burst: 9, priority: 3 }
			]
		},
		mixedArrival: {
			count: 4,
			quantum: 2,
			processes: [
				{ arrive: 0, burst: 5, priority: 3 },
				{ arrive: 1, burst: 3, priority: 1 },
				{ arrive: 2, burst: 1, priority: 2 },
				{ arrive: 3, burst: 4, priority: 4 }
			]
		},
		sjfCompare: {
			count: 4,
			quantum: 2,
			processes: [
				{ arrive: 0, burst: 7, priority: 4 },
				{ arrive: 2, burst: 4, priority: 2 },
				{ arrive: 3, burst: 1, priority: 1 },
				{ arrive: 4, burst: 4, priority: 3 }
			]
		},
		roundRobinDemo: {
			count: 4,
			quantum: 2,
			processes: [
				{ arrive: 0, burst: 5, priority: 1 },
				{ arrive: 1, burst: 4, priority: 2 },
				{ arrive: 2, burst: 2, priority: 3 },
				{ arrive: 3, burst: 1, priority: 4 }
			],
			selectAlgorithm: 'Round Robin'
		},
		priorityDemo: {
			count: 4,
			quantum: 2,
			processes: [
				{ arrive: 0, burst: 4, priority: 2 },
				{ arrive: 1, burst: 3, priority: 1 },
				{ arrive: 2, burst: 2, priority: 4 },
				{ arrive: 3, burst: 5, priority: 3 }
			],
			selectAlgorithm: 'Priority'
		}
	};

	function applyPresetRows(count) {
		for (var r = 1; r <= 10; r++) {
			if (r <= count) {
				$("#row_" + r).collapse("show");
			} else {
				$("#row_" + r).collapse("hide");
			}
		}
		$('#remove_row').prop("disabled", count <= 1);
		$('#add_row').prop("disabled", count >= 10);
	}

	window.loadCpuPreset = function (presetId) {
		var preset = CPU_PRESETS[presetId];
		if (!preset) {
			return;
		}

		processCount = preset.count;
		$('#proccess_num').val(processCount);
		applyPresetRows(processCount);

		for (var i = 0; i < preset.processes.length; i++) {
			var p = preset.processes[i];
			var idx = i + 1;
			$('#arrive_' + idx).val(p.arrive);
			$('#burst_' + idx).val(p.burst);
			$('#priority_' + idx).val(p.priority);
		}

		if (preset.quantum) {
			timeQuantum = preset.quantum;
			$('#enter_quantum').val(preset.quantum);
			tq = preset.quantum;
		}

		if (preset.selectAlgorithm) {
			algorithm = preset.selectAlgorithm;
			$("#algorithm_button").html(algorithm + ' <span class="caret">');
			if (algorithm === "Round Robin") {
				$("#solver_group").removeClass("hidden");
			} else {
				$("#solver_group").addClass("hidden");
	...
	window.setSchedulingAlgorithm = function (algName) {
		algorithm = algName;
		Selectedalgorithm = algName;
		$("#algorithm_button").html(algName + ' <span class="caret">');
		if (algName === "Round Robin") {
			$("#solver_group").removeClass("hidden");
		} else {
			$("#solver_group").addClass("hidden");
		}
		...
		run();
		return window.lastSimulationSnapshot;
	};

	window.compareAllAlgorithms = function () {
		var algorithms = ["FCFS", "SJF", "SRJF", "Round Robin", "Priority"];
		var saved = algorithm;
		var results = [];
		for (var i = 0; i < algorithms.length; i++) {
			window.setSchedulingAlgorithm(algorithms[i]);
			if (window.lastSimulationSnapshot) {
				results.push(Object.assign({}, window.lastSimulationSnapshot));
			}
		}
		algorithm = saved;
		window.setSchedulingAlgorithm(saved);
		return results;
	};

	window.exportSimulationResults = function () {
		if (!mainOutput.o_pid || mainOutput.o_pid.length === 0) {
			alert('Please run the simulation first (click Run).');
			return;
		}

		var lines = [];
		lines.push('CPU Scheduling Simulation Export');
		lines.push('Algorithm,' + (mainOutput.algorithm || Selectedalgorithm || ''));
		lines.push('Average Waiting Time,' + mainOutput.avgWait);
		lines.push('Average Turnaround Time,' + mainOutput.avgtat);
		lines.push('CPU Utilization (%),' + mainOutput.utilization);
		if (mainOutput.quantum) {
			lines.push('Time Quantum,' + mainOutput.quantum);
		}
		lines.push('');
		lines.push('Process,Arrival Time,Burst Time,Completion Time,Turnaround Time,Waiting Time');

		for (var j = 0; j < mainOutput.o_pid.length; j++) {
			lines.push([
				mainOutput.o_pid[j],
				mainOutput.o_arrivaltime[j],
				mainOutput.o_bursttime[j],
				mainOutput.completionTime[j],
				mainOutput.turnAroundTime[j],
				mainOutput.waitingTime[j]
			].join(','));
		}
		...
		link.download = 'cpu-scheduling-' + (mainOutput.algorithm || 'result') + '-' + stamp + '.csv';
...
$(".runButton").click(function () {
	...
	mainOutput.algorithm = Selectedalgorithm || '';
	...
	let metricAlgorithm = document.getElementById("metricAlgorithm");
	if (metricAlgorithm) {
		metricAlgorithm.textContent = mainOutput.algorithm || Selectedalgorithm || '—';
	}

	let metricsSummary = document.getElementById("metricsSummary");
	if (metricsSummary) {
		metricsSummary.style.display = "flex";
	}
